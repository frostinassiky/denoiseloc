...
class SoftNMSop(torch.autograd.Function):
    @staticmethod
    def forward(ctx, segs, scores, cls_idxs, iou_threshold, sigma, min_score, method, max_num):
...
def batched_nms1d(
    ...
    if multiclass:
        ...
        if use_soft_nms:
            ...
            # here, 2 means we use gaussian soft-nms, so iou_threshold does not matter
    ...
    return new_segs, new_scores, new_cls_idxs,idxs[:max_seg_num]
...
if __name__ == "__main__":
    ...
    segments, scores, labels = batched_nms1d(
        segments,
        scores,
        cls_idxs=torch.ones_like(scores),
        iou_threshold=0.5,
        min_score=0,
        max_seg_num=2,
        use_soft_nms=True,
